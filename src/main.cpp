...
const string strMessageMagic = "VARIUS Signed Message:\n";
...
static unsigned int GetNextTargetRequiredV1(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    int64_t nInterval = nTargetTimespan_v1 / nTargetSpacing_v1;
    bnNew *= ((nInterval - 1) * nTargetSpacing_v1 + nActualSpacing + nActualSpacing);
    bnNew /= ((nInterval + 1) * nTargetSpacing_v1);
...
bool CBlock::AcceptBlock(CNode* pfrom, CBlock* pblock)
{
    AssertLockHeld(cs_main);
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Store to disk
    if (!pblock->AcceptBlock(pfrom, pblock))
    ...
    for (unsigned int i = 0; i < vWorkQueue.size(); i++)
    {
             ...
             ++mi)
        {
            ...
            if (pblockOrphan->AcceptBlock(pfrom, pblock))
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "VARIUS", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
